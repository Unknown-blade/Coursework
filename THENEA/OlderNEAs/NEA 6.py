...
class Stage0(World):
    ...
    def draw_stage0(self):
        self.surface_list.empty()
        for n in range(22):
            block = Ground()
            block.rect.left = n * 63
            block.rect.top = WINDOW_HEIGHT - 63
            self.surface_list.add(block)
...
class CameraGroup(pygame.sprite.Group, World):
    ...
    def draw_stages(self,current_stage,ground):
        self.stage0 = Stage0(player)
        self.surface_list = stage0.draw_stage0()
        for surfaces in self.surface_list:
            stageoffsets = surfaces.rect.center-self.offset
            screen.blit(surfaces.ground_image,stageoffsets)
